...
async function c_tester(circomInput, _options) {
    ...
    if (options.compile) {
        ...
        const cppPath = path.join(options.output, baseName + "_cpp");
        try {
            await fs.promises.access(cppPath);
        } catch (err) {
            assert(false, "Cannot set recompile to false if the " + cppPath + " folder does not exist");
        }
    }
